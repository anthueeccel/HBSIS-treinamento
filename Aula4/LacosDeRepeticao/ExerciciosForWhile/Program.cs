using System;
/// <summary>
/// Código para estudo de For e While
/// Criado por: Anthue Eccel
/// Data: 2019-09-12
/// </summary>
...
namespace ExerciciosForWhile
{
    class Program
    {
        ...
        {
            Console.WriteLine("Escolha a opção para a busca [1] FOR ou [2] WHILE");
            var opcao = Console.ReadKey().KeyChar.ToString();

            switch (opcao)
            {
                case "1":
                    Console.Write("(FOR) Digite a palavra a ser contada: ");
                    string wordToFind = Console.ReadLine();
                    Console.WriteLine($"Quantidade de palavras '{wordToFind}' encontradas: {FindByFor(wordToFind)}");
                    break;
                case "2":
                    Console.Write("(WHILE) Digite a palavra a ser contada: ");
                    string wordToFind2 = Console.ReadLine();
                    Console.WriteLine($"Quantidade de palavras '{wordToFind2}' encontradas: {FindByWhile(wordToFind2)}");
                    break;
                default:
                    break;
            }
        ...
        /// <summary>
        /// Método de busca de palavra dentro da aplicação
        /// </summary>
        /// <param name="wordToFind">Parâmetro de 3 caracteres</param>
        ...
        private static int FindByFor(string wordToFind)
        {
            ...
            var wordToFindLength = (wordToFind.Length);
            var contentLength = (contentToFind.Length - 2);
            int wordCounter = 0;
            ...
            for (int i = 0; i < contentLength; i++)
            {
                ...
                {
                    compareWord += contentToFind[i + j].ToString();
                }
        ...
        /// <summary>
        /// Método para contar a quantidade de palavras em um texto (WHILE)
        /// </summary>
        /// <param name="wordToFind">Parâmetro de 3 caracteres</param>
        /// <returns>Quantidade de palavras encontradas</returns>
        private static int FindByWhile(string wordToFind)
        {
            var contentToFind = TextContent();
            var wordToFindCountLegth = wordToFind.Length;
            var contentLength = (contentToFind.Length - (wordToFindCountLegth - 1));
            int wordCounter = 0;
            int j = 0;
            ...
            {
                var palavraParaComparar = string.Empty;
                var b = 0;
                while (b < wordToFindCountLegth)
                {
                    palavraParaComparar += wordToFind[j + b].ToString();
                    b++;
                }

                if (wordToFind == palavraParaComparar)
                    wordCounter++;
        ...
        /// </summary>
        /// <returns>O conteudo do texto e String</returns>
